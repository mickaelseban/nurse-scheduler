...
import domain.DomainError.IOFileProblem
import io.FileIO.*
...
trait AssessmentBehaviours extends AnyFunSuite:
  ...
  val OK = "OK"
  val FAILURE = "FILE DOES NOT CONTAIN MESSAGE!"
  ...
  private def testWithFailure(d: DomainError, f: File): (File, String) =
    ...
    loadError(efn).fold(_ => (f, FAILURE), s => if (s == d.toString) (f, OK) else (f, s"Expected: $s ; Result: $d") )
